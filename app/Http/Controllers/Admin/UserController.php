...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($user->save()) {
                ...
                if ($request->has('company') && is_array($request->get('company'))) {
    ...
    public function edit($id)
    {
        if (auth()->user()->can('customer-edit')) {
            ...
            ->get();
            $selectedCompanies = $data->companies; // Assuming a relationship exists

            return view('admin.users.edit', compact('data','currentBrothers','selectedCompanies'));
        } else {
            return redirect()->back()
        ...
        try {
            ...
            if ($user->save()) {
                ...
                if ($request->has('family_id') && count($request->get('family_id')) > 0) {
                   ...
                   // Update companies for the user
            if ($request->has('company') && is_array($request->get('company'))) {
                // Sync companies (remove old associations and add new ones)
                $userCompanies = $request->get('company');
                \DB::table('user_companies')
                    ->where('user_id', $user->id)
                    ->whereNotIn('company_id', $userCompanies)
                    ->delete();
