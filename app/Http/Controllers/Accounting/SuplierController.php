...
use App\Models\SuplierCategory;
use App\Models\User;
...
class SuplierController extends Controller
{
   protected $columnsIndex;
    ...
    {
        $this->index_var = "Accounting-suplier-table";
        $this->create_var = "Accounting-suplier-add";
        $this->edit_var = "Accounting-suplier-edit";
        $this->delete_var = "Accounting-suplier-delete";
        ...
        [
            ...
            __('messages.multiple_branch'),
            __('messages.branch'),
        ];
        $this->columns_table_name = ['number', 'name', 'foreign_name','address','tax_number','phone','mobile','tax_status','multiple_branch','branch_id'];
        $this->inputTypes = ['text', 'text','text','text', 'text','text','text','options','options', 'select_another_table'];
        $this->required = ['required', 'required',null,null,null,null,null,null,null,null];
        $this->options = [
            ['1' => 'Taxable', '2' => 'Non Taxable', '3' => 'Zero Tax', '4' => 'Trans', '5' => 'Not Registered'],
            ['1' => 'Active', '2' => 'Inactive'],

        ];
        $this->routeName = 'supliers'; // Change to your route name
        $this->modelName = Suplier::class; // Change to your model name
        $this->model_name_for_another_table_select = Branch::class; // Change to  model name
        $this->currentApplicationName = "Accounting";
        $this->var_releation_radio_tree = 'suplierCategory_id'; // هون بنحط اسم الكولومن الموجود في الداتابيس تاع هاد الموديل
        ...
        $this->another_table_for_radio =SuplierCategory::class; // keep it null . if there is another model change it
