...
use App\Models\AddShiftToEmployee;
...
class AddShiftToEmployeeController extends Controller
{
   protected $columnsIndex;
    ...
    {
        $this->index_var = "PayRolls-addShiftToEmployee-table";
        $this->create_var = "PayRolls-addShiftToEmployee-add";
        $this->edit_var = "PayRolls-addShiftToEmployee-edit";
        $this->delete_var = "PayRolls-addShiftToEmployee-delete";

        $this->columnsIndex = ['id','from_date', 'to_date'];
        $this->columns_view =
        [
            __('messages.Number'),
            __('messages.Name'),
        ...
        $this->routeName = 'addShiftToEmployees'; // Change to your route name
        $this->modelName = AddShiftToEmployee::class; // Change to your model name
    ...
    public function create(Request $request)
    {
        if (auth()->user()->can($this->create_var))
        {
            ...
            $shifts = Shift::get();
            return view('custom_pages.shiftToEmployees.create',compact('employees','shifts'))->with(['currentApplicationName' => $this->currentApplicationName,]);
    ...
    public function store(Request $request)
    {
             ...
             $request->validate([
                ...
                'shifts.*' => 'required|exists:shifts,id', // Ensure each shift ID exists in the shifts table
            ]);
            ...
            foreach ($shifts as $employeeId => $shiftId) {
                // Create or update the shift assignment
                AddShiftToEmployee::updateOrCreate(
